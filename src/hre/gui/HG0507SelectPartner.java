 ...
 */
	...
	// Load the role lists for chosen Event type
		...
		if (partnerRelationData != null) lbl_nRole1 = new JLabel("" + partnerRelationData[4]);	//$NON-NLS-1$
		else lbl_nRole1 = new JLabel("");	//$NON-NLS-1$
			...
			public void actionPerformed(ActionEvent arg0) {		
				...
				try {
					...
					// Add new partner event
						...
						if (memoEdited) pointPersonHandler.updateSelectGUIMemo(memoText.getText(), 
 ...
 */
    ...
    // Set edit label for partner2	    	
    	...
    	for (int i = 0; i < partnerTypeNumbers.length; i++) {
    		if (partnerTypeNumbers[i] == partnerRoleNumber) partnerRoleindex = i;	
    	}
    	comboBox_Relationships.setSelectedIndex(partnerRoleindex);
    	
    	for (int i = 0; i < partnerRoleType.length; i++) {
    		if (partnerRoleType[i] == priPartnerRole) priPartnerRoleIndex = i;	
    	}
    	comboPartRole1.setSelectedIndex(priPartnerRoleIndex);
    	
    	for (int i = 0; i < partnerRoleType.length; i++) {
    		if (partnerRoleType[i] == secPartnerRole) secPartnerRoleIndex = i;	
    	}
