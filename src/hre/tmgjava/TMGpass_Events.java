...
class TMGpass_Events  {
	...
	// control of role sentence console printout
	boolean printRoleSentence = false;
 ...
 * and is the two dimensional array holding pri_role and sec_role for parner relation
 */
...
/**
 * ******************************* Check these language codes
 */
	String[] subjectTranslate = {"Subject","Subject","Sujet","Onderwerp","Subjekt","Emne"};
	String[] standardLanguage = {"en-US","en-GB","fr-FR","nl-NL","de-DE","no-NB"};
 ...
 */
	public void addToT450_EVNT(int rowPID, int etypeNumber, long locationTablePID, ResultSet hreTable) throws HCException {
		...
		//Insert row
			...
			System.out.println("TMGPass_Names - addTo table T450_EVNT - error: " + sqle.getMessage());
			sqle.printStackTrace();
			throw new HCException("TMGPass_Names - addTo table T450_EVNT - error: " + sqle.getMessage());
		}
	}
 ...
 */
    protected void addToT404_PARTNER(int primaryIndex, int indexG_PID, ResultSet hreTable) throws HCException {
		...
		//Insert row
			...
			System.out.println("TMGPass_Names - addTo table addToT404_PARTNER - error: "
					+ sqle.getMessage());
			sqle.printStackTrace();
			throw new HCException("TMGPass_Names - addTo table addToT404_PARTNER - error: "
					+ sqle.getMessage());
		}
    }
 ...
 */
		...
		// DATASET CHECK
			...
			// else do user-added (origetype = 0)
				...
				// Add rolecode = 0 and frefix subject translation
					for (int i = 0; i < standardLanguage.length; i++) {
						eventRolePID++;
						addToT461_EVNT_ROLE(eventRolePID, eventTypeNr, 0, 0,
								subjectTranslate[i], standardLanguage[i], tableT461);
						//System.out.println(" Event role PID: " + eventRolePID + "-"
						//		+ eventTypeNr + "/" + subjectTranslate[i]
						//		+ "/" + standardLanguage[i]);
					}
				 }

			// Print role sentences on console
				if (printRoleSentence) extractRoleSentence(eventTypeNr, etypeName, tsentence);
			// Extract roles
				extractRoles(eventTypeNr, tsentence, tableT461);
			}
		}
	}

/**
 * extractRoles(String eventName, String roleString)
 * @param roleString
 * @throws HCException
 */
	private void extractRoles( int eventType,
		...
		// Now split the rile tranlation string
...
/**
 * extractRoleSentence(int eventTypeNr, String etypeName, String roleString)
 * The method parse the tsentense field in T.dbf and prepare for printout of role and sentences
 * @param eventTypeNr
 * @param etypeName
 * @param roleString
 */
	private void extractRoleSentence(int eventTypeNr, String etypeName, String roleString) {
		System.out.println(" ROLES for EVENT nr: " + eventTypeNr + " name: " + etypeName);
		String roleSentenceString;
		int pos = roleString.indexOf("[:LABELS]");
		if (roleString.length() == 0) return;
		roleSentenceString = roleString.substring(pos, roleString.length());
		...
		for (int i = 1; i < sentenceLaguages.length; i++) {
			String stringLine = sentenceLaguages[i].substring(0, sentenceLaguages[i].length() - 1);
			//System.out.println(i + " - " + stringLine);
			String[] roleSentence = stringLine.split("R=");
			String language = roleSentence[0].substring(0, roleSentence[0].length() - 2);
			if (getLangCode(language).equals("en-US")) {
				System.out.println(" Language: " + i + " - " + language + " Code: " + getLangCode(language));
				for (int j = 1; j < roleSentence.length; j++) {
					String roleNumber = roleSentence[j].substring(0,5);
					System.out.print(" Role: " + roleNumber);
					int endposValue;
					if (j < roleSentence.length-1) endposValue = 1; else  endposValue = 0;
					String roleTMGsentence = roleSentence[j].substring(6, roleSentence[j].length() - endposValue);
 ...
 */
	protected void updateT460_EVNT_DEFS(int etypeNumber, int origeType, String etypeName) throws  HCException {
		...
		try {
			...
			if (TMGglobal.DEBUG)
				System.out.println(" updateT460_EVNT_DEFS error: " + sqle.getMessage());
			throw new HCException(" updateT460_EVNT_DEFS error: " + sqle.getMessage());
		}
 ...
 */
	...
	// Abbrev too long:
		...
		//Insert row
			...
			System.out.println("TMGPass_Names - addTo table T460_EVNT_DEFN - error: " + sqle.getMessage());
			sqle.printStackTrace();
			throw new HCException("TMGPass_Names - addTo table T460_EVNT_DEFN - error: " + sqle.getMessage());
		}
	}
 ...
 */
	protected void addToT461_EVNT_ROLE(long primaryPID,
		...
		//Insert row
			...
			sqle.printStackTrace();
			throw new HCException("TMGPass_Names - addTo table T461_EVNT_ROLE - error: "
					+ sqle.getMessage());
		}
	}
 ...
 */
		...
		// Extract the partner roles from witness table e.dbf
			...
			// Exclude self associate if per1 same as person and per2 not set and not primary
				//if ((eper == per1 && per2 == 0 && !primary)) { // Default 80 assocs for Don	
 ...
 */
 ...
 // Witness Sentence not used - TMG sentence format
		...
		//Insert row
			...
			System.out.println("TMGPass_Names - addTo table T451_EVNT_ASSOC - error: " + sqle.getMessage());
			sqle.printStackTrace();
			throw new HCException("TMGPass_Names - addTo table T451_EVNT_ASSOC - error: " + sqle.getMessage());
		}
    }
  ...
  */
    public void updatePartnerRoles() throws HCException {
    	...
    	try {
			...
			System.out.println(" Update updatePartnerRoles(): " + sqle.getMessage());
			sqle.printStackTrace();
		}
    }
