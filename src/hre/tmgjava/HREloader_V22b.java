 ...
 * @throws HCException
 */
	public HREloader_V22b(String urlH2loc, TMGHREconverter tmgHreConverter) throws HCException {
		int nrOfTables = 24;
		int completed = 0;
		...
		try {
			...
			System.out.println(" HREloader_V22b - Connect HRE database!");

			pointHREbase = new HREdatabaseHandler(urlH2loc);
			
	// Temporary add columns to table T126 - IS_IMPORTED
			alterColumnInTable("T126_PROJECTS","IS_IMPORTED","BOOLEAN");
			updateTableInBase("T126_PROJECTS", "UPDATE", "SET IS_IMPORTED = TRUE WHERE PROJECT_CODE = 1");	
			updateTableInBase("T170_DATE", "ALTER TABLE", "ALTER HDATE_DETAILS CHARACTER(50)");	
			
	// Partners		
			alterColumnInTable("T404_PARTNER","EVNT_RPID","BIGINT");
			alterColumnInTable("T404_PARTNER","PRI_ROLE","SMALLINT");
			alterColumnInTable("T404_PARTNER","SEC_ROLE","SMALLINT");
			//alterColumnInTable("T404_PARTNER","IMP_TMG","BOOLEAN");
			
	// Events
			//alterColumnInTable("T450_EVNT","IMP_TMG","BOOLEAN");
			
	//Exhibits temp changes until CAPTION and DESCRIPTION are implemented as for v22b
			alterColumnInTable("T677_DIGT_NAME","CAPTION", "VARCHAR(150)");
			alterColumnInTable("T677_DIGT_NAME","DESCRIPTION","VARCHAR(500)");
			
	// Temporary add columns to table
			alterColumnInTable("T676_DIGT","SUB_TYPE_RPID","BIGINT");		
